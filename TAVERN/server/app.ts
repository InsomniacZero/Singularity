...
/**
 * Automatically illustrates scenes and locations by connecting to Singularity Gateway (/v1/images/generations).
 * Replaces the legacy universal-gift (8045/scene/illustrate) service.
 */
app.post(['/api/scene/illustrate', '/scene/illustrate'], async (req, res) => {
  const { text, scene, prompt, location, chatId, chat_id, timePhase, time_phase, force, model } = req.body || {}
  const rawText = (text || scene || prompt || '').trim()
  const loc = (location || '').trim()
  const cid = (chatId || chat_id || '').trim()
  const timeStr = (timePhase || time_phase || '').trim()

  // 1. Check existing chat locationArt cache if chat_id and location provided and not forced
  if (!force && cid && loc) {
    try {
      const row = db.prepare('SELECT data FROM chats WHERE id = ?').get(cid) as { data: string } | undefined
      if (row?.data) {
        const parsed = JSON.parse(row.data)
        if (parsed.locationArt && parsed.locationArt[loc]) {
          res.json({
            url: parsed.locationArt[loc],
            cached: true,
            location: loc,
            timePhase: timeStr,
            success: true,
  ...
  // 2. Clean narrative text strictly for environment cues (strip dialogue in quotes/brackets/markdown)
  let cleanText = rawText
    .replace(/<{1,2}scene:[^>]*>{1,2}/gi, '')
    .replace(/!\[.*?\]\(.*?\)/g, '')
    .replace(/\[.*?\]\(.*?\)/g, '')
    .replace(/["“「][^"”」]*["”」]/g, '')
    .replace(/[*_~`]/g, '')
  ...
  const envCues: string[] = []
  if (loc) envCues.push(`Setting: ${loc}`)
  if (timeStr) envCues.push(`Time of day: ${timeStr}`)
  if (cleanText) envCues.push(`Environment: ${cleanText}`)
  const envDesc = envCues.length > 0 ? envCues.join('. ') : 'scenic anime visual novel setting'

  const imagePrompt =
    `Generate an image: visual novel anime digital scenery background, cinematic lighting, masterpiece, 1080p widescreen wallpaper. ${envDesc}. ` +
  ...
  const singularityUrl = (process.env.SINGULARITY_URL || 'http://127.0.0.1:9000/v1').replace(/\/+$/, '')
  const selectedModel = model || 'gemini-3.8-flash'

  try {
    const sResp = await fetch(`${singularityUrl}/images/generations`, {
      method: 'POST',
    ...
    if (!sResp.ok) {
      ...
      res.status(502).json({ error: `Singularity image error (${sResp.status}): ${errText.slice(0, 200)}` })
      return
    }

    const data = (await sResp.json()) as { data?: { b64_json?: string; url?: string }[] }
    const first = data?.data?.[0]
    let finalUrl = ''

    if (first?.b64_json) {
      const scenesDir = path.join(avatarsDir, 'scenes')
      fs.mkdirSync(scenesDir, { recursive: true })
      const filename = `scene-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.png`
      fs.writeFileSync(path.join(scenesDir, filename), Buffer.from(first.b64_json, 'base64'))
      finalUrl = `/avatars/scenes/${filename}`
    } else if (first?.url) {
      finalUrl = first.url
    }
    ...
    console.error('Scene illustrate error:', err)
