...
import { extractEarlySceneTag, extractSceneTag, stripSceneTagForDisplay, type SceneTag } from '@/lib/vn/sceneTag'
import { inferExpressionFromText } from '@/lib/vn/expressionInference'
...
import { backgroundLabel, DEFAULT_BACKGROUNDS, matchBackgroundKeyword } from '@/lib/vn/backgrounds'
...
export function useChatSession(chatId: string | null) {
  ...
  const refineExpressionFromText = useCallback(
    async (messageId: string, speaker: Character, replyText: string) => {
      ...
      let currentExp = currentScene.expression
      if (!currentExp || currentExp === 'neutral') {
        const inferred = inferExpressionFromText(replyText, unlockedExpressions)
        if (inferred && inferred !== currentExp) {
          const next: SceneTag = { ...currentScene, expression: inferred }
          const sanitized = sanitizeSceneTag(next, unlockedExpressions, unlockedBackgrounds, selectableOutfits)
          if (sanitized) {
            const swipeScenes = freshMsg.swipeScenes ? [...freshMsg.swipeScenes] : []
            swipeScenes[activeSwipe] = sanitized
    ...
    async (
      ...
      try {
        ...
        for (let round = 0; round <= MAX_AUTO_CONTINUE_ROUNDS; round++) {
          ...
          let earlyTriggeredLoc: string | null = null
          const earlyIllustrationRef: { promise: Promise<string | null> | null } = { promise: null }

          const checkAndTriggerEarlyIllustration = (streamedChunkText: string) => {
            if (earlyTriggeredLoc || !useSettingsStore.getState().autoSceneIllustration) return

            let candidateLocation: string | undefined

            // 1. Scene tag at start or mid-stream (e.g. <<scene:expression=happy,background=library>>)
            const earlyTag = extractEarlySceneTag(streamedChunkText)
            if (earlyTag?.background) {
              candidateLocation = backgroundLabel(earlyTag.background, world) || earlyTag.background
            } else if (streamedChunkText.length >= 30 && streamedChunkText.length <= 350) {
              // 2. Early natural narrative keyword matching against candidate world/default locations
              const matchedId = matchBackgroundKeyword(streamedChunkText, [
                ...DEFAULT_BACKGROUNDS,
                ...(world?.customBackgrounds ?? []).map((b) => ({ id: b.id, label: b.label })),
              ])
              if (matchedId) {
                candidateLocation = backgroundLabel(matchedId, world) || matchedId
              }
            }

            if (!candidateLocation) return

            const normLoc = candidateLocation.trim().toLowerCase()
            const currentSceneLoc = (chat.scene?.location ?? '').trim().toLowerCase()

            earlyTriggeredLoc = normLoc

            // 1. If location already has art in memory, swap instantly (0ms) and update scene location
            const existingMemoryArt = chat.locationArt?.[normLoc]
            if (existingMemoryArt) {
              if (normLoc !== currentSceneLoc) {
                void chatsApi.update(chat.id, {
                  scene: { ...(chat.scene ?? { turnPolicy: 'manual' }), location: candidateLocation },
                })
              }
              return
            }

            // 2. New location: launch early parallel generation!
            earlyIllustrationRef.promise = (async () => {
              try {
                const res = await fetch('/api/scene/illustrate', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({
                    text: streamedChunkText,
                ...
                if (res.ok) {
                  ...
                  if (data?.url) {
                    ...
                    await chatsApi.update(chat.id, {
                      locationArt: updated,
                      scene: { ...(fresh?.scene ?? { turnPolicy: 'manual' }), location: candidateLocation },
                    })
                    return data.url
                  }
                }
              } catch (err) {
                console.warn('[Nano-Banana] Early stream illustration failed:', err)
              }
              return null
            })()
          }
          ...
          try {
            newText = await client.generateStream(
              ...
              (_token, full) => {
                ...
                checkAndTriggerEarlyIllustration(full)
              },
              abort.signal,
            )
          } catch (streamErr) {
          ...
          if (!scene?.expression || scene.expression === 'neutral') {
            const inferred = inferExpressionFromText(combined, unlockedExpressions)
            if (inferred) {
              scene = { ...(scene ?? {}), expression: inferred }
            }
          }
          ...
          // Fire-and-forget: Auto-illustrate scene with Nano Banana in background
          if (isUsableReply && useSettingsStore.getState().autoSceneIllustration) {
            const textToIllustrate = combined
            const locationName = chat.scene?.location || scene?.background || 'classroom'
            const normLoc = locationName.trim().toLowerCase()
            const existingArt = chat.locationArt?.[normLoc]

            if (existingArt) {
              // Location already in memory! Reuse immediately for 100% visual continuity and 0 ms latency
              void messagesApi.update(targetMessageId, { images: [existingArt] })
            } else if (earlyIllustrationRef.promise) {
              // Early stream-triggered generation is already cooking! Attach the resulting URL as soon as ready
              void earlyIllustrationRef.promise.then(async (url: string | null) => {
                if (url) {
                  await messagesApi.update(targetMessageId, { images: [url] })
                }
              })
            } else {
              // Fallback if new location wasn't detected during early stream chunks
              void (async () => {
                try {
                  const res = await fetch('/api/scene/illustrate', {
                    method: 'POST',
                  ...
                  if (res.ok) {
                    const data = (await res.json()) as { url?: string }
                    if (data?.url) {
                      await messagesApi.update(targetMessageId, { images: [data.url] })
                      const fresh = await chatsApi.get(chat.id)
                      const updated = { ...(fresh?.locationArt || {}), [normLoc]: data.url }
                  ...
                  console.warn('[Nano-Banana] Auto scene illustration fallback failed:', err)
                }
              })()
            }
          }
          ...
          const hitCap = !abort.signal.aborted && generatedTokens >= effectiveMaxLength - 1 && !endsCleanly(combined)
          const endedMidThought =
            round === 0 && !abort.signal.aborted && !!newText.trim() && !hitCap && !endsCleanly(combined)
          // Extend a reply that ran into the token budget mid-sentence, but never for complete sentences
          // or modern chat-completion backends which return full conversational turns.
          const looksTruncated =
            chatBackend !== 'openai-compatible' &&
            !endsCleanly(combined) &&
            !bandCapsBelowUserMax &&
            (hitCap || endedMidThought)
