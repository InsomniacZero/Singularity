...
import { DEFAULT_THEME_TOKENS, DEFAULT_THEME_TOKENS_DARK, useSettingsStore } from '@/lib/store/useSettingsStore'
...
export function useApplyTheme() {
  ...
  useEffect(() => {
    ...
    const fallback = colorMode === 'dark' ? DEFAULT_THEME_TOKENS_DARK : DEFAULT_THEME_TOKENS
    const tokens = (colorMode === 'dark' ? themeTokensDark : themeTokensLight) || fallback
    for (const [key, value] of Object.entries(tokens ?? fallback)) {
