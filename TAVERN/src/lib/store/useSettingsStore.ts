...
interface SettingsState {
  ...
  /** Automatically illustrate RP turns and scenes using Nano Banana */
  autoSceneIllustration: boolean
  setAutoSceneIllustration: (enabled: boolean) => void
...
export const useSettingsStore = create<SettingsState>()(
  persist(
    (set) => ({
      ...
      chatBackendBaseUrl: 'http://localhost:9000/v1',
      chatBackendApiKey: '',
      chatBackendModel: 'gemini-3.8-flash',
      ...
      autoSceneIllustration: true,
      setAutoSceneIllustration: (enabled) => set({ autoSceneIllustration: enabled }),

      imageBackend: 'singularity',
      imageBackendBaseUrl: 'http://localhost:9000/v1',
      imageBackendUsername: '',
      imageBackendPassword: '',
      imageBackendModel: 'gemini-3.8-flash',
    ...
    }),
    {
      ...
      merge: (persisted, current) => {
        ...
        // Auto-upgrade legacy universal-gift (8045) references to Singularity Gateway (9000)
        let baseUrl = (typeof p.baseUrl === 'string' ? p.baseUrl : current?.baseUrl) ?? 'http://localhost:9000/v1'
        let chatBackendBaseUrl = (typeof p.chatBackendBaseUrl === 'string' ? p.chatBackendBaseUrl : current?.chatBackendBaseUrl) ?? 'http://localhost:9000/v1'
        let imageBackendBaseUrl = (typeof p.imageBackendBaseUrl === 'string' ? p.imageBackendBaseUrl : current?.imageBackendBaseUrl) ?? 'http://localhost:9000/v1'
        let imageBackend = p.imageBackend ?? current?.imageBackend ?? 'singularity'
        let chatBackendModel = p.chatBackendModel ?? current?.chatBackendModel ?? 'gemini-3.8-flash'

        if (baseUrl.includes(':8045')) baseUrl = 'http://localhost:9000/v1'
        if (chatBackendBaseUrl.includes(':8045')) {
          chatBackendBaseUrl = 'http://localhost:9000/v1'
          if (chatBackendModel === 'gemini-3.1-pro-extended') chatBackendModel = 'gemini-3.8-flash'
        }
        if (imageBackendBaseUrl.includes(':8045') || imageBackendBaseUrl.includes(':8000')) imageBackendBaseUrl = 'http://localhost:9000/v1'
        if (imageBackend === 'universal-gift' || (imageBackend as string) === 'chatgpt-image') imageBackend = 'singularity'

        const darkTokens = { ...current.themeTokensDark, ...p.themeTokensDark }
        if (darkTokens['--c-accent'] === '129 178 154' || darkTokens['--c-accent'] === '61 107 86') {
          darkTokens['--c-accent'] = '217 119 87'
          darkTokens['--c-accent-text'] = '255 255 255'
        }
        if (darkTokens['--c-bg'] === '26 24 22') {
          darkTokens['--c-bg'] = '21 21 21'
          darkTokens['--c-bg-elevated'] = '45 43 41'
          darkTokens['--c-bg-sunken'] = '25 24 23'
          darkTokens['--c-border'] = '43 40 38'
        }

        const lightTokens = { ...current.themeTokensLight, ...p.themeTokensLight }
        if (lightTokens['--c-accent'] === '61 107 86') {
          lightTokens['--c-accent'] = '217 119 87'
        }
        if (lightTokens['--c-bg'] === '250 248 245') {
          lightTokens['--c-bg'] = '252 252 251'
          lightTokens['--c-border'] = '231 229 225'
        }
        ...
        return {
          ...
          themeTokensLight: lightTokens,
          themeTokensDark: darkTokens,
