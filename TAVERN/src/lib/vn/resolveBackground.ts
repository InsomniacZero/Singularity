...
export function resolveSceneBackground(params: ResolveBackgroundParams): ResolvedBackground {
  ...
  if (taggedBackground && unlocked(taggedBackground, world, affection)) {
    const art = withArt(taggedBackground, 'tag')
    if (art.url) return art
    if (chat.locationArt?.[taggedBackground.toLowerCase()]) {
      return { id: taggedBackground, url: chat.locationArt[taggedBackground.toLowerCase()], source: 'tag' }
    }
    return art
  }
  ...
  if (chat.scene?.location) {
    ...
    if (fromLocation && unlocked(fromLocation, world, affection)) {
      const art = withArt(fromLocation, 'location')
      if (art.url) return art
      const locKey = fromLocation.toLowerCase()
      if (chat.locationArt?.[locKey]) return { id: fromLocation, url: chat.locationArt[locKey], source: 'location' }
      const exactLoc = chat.scene.location.trim().toLowerCase()
      if (chat.locationArt?.[exactLoc]) return { id: fromLocation, url: chat.locationArt[exactLoc], source: 'location' }
      return art
    }
    const customKey = chat.scene.location.trim().toLowerCase()
    if (chat.locationArt?.[customKey]) {
      return { id: customKey, url: chat.locationArt[customKey], source: 'location' }
    }
  }
