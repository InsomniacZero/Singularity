...
describe('resolveSceneBackground', () => {
  ...
  it('resolves remembered 1080p backdrop from chat.locationArt for the active scene location', () => {
    const c = chat({
      scene: { turnPolicy: 'manual', location: 'School Hallway' },
      locationArt: { 'school hallway': 'https://files.catbox.moe/hallway.png' },
    })
    const out = resolveSceneBackground({ chat: c, affection: 0 })
    expect(out).toEqual({ id: 'school-hallway', url: 'https://files.catbox.moe/hallway.png', source: 'location' })
  })
