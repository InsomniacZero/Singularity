...
import { buildSceneInstruction, extractEarlySceneTag, extractSceneTag } from '@/lib/vn/sceneTag'
...
describe('extractEarlySceneTag', () => {
  it('extracts background from complete or incomplete tags early in stream', () => {
    const complete = extractEarlySceneTag('<<scene:expression=smile,background=library>>\nHello')
    expect(complete?.background).toBe('library')
    expect(complete?.expression).toBe('smile')

    const incomplete = extractEarlySceneTag('<<scene:expression=smile,background=classroom')
    expect(incomplete?.background).toBe('classroom')
    expect(incomplete?.expression).toBe('smile')
  })
})
