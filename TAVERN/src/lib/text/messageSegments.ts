export type MessageSegment = { type: 'text' | 'action' | 'quote' | 'sfx' | 'image'; content: string; alt?: string; url?: string }
...
function coalesce(segments: MessageSegment[]): MessageSegment[] {
  ...
  for (const seg of segments) {
    ...
    if (last && last.type === seg.type && seg.type !== 'sfx' && seg.type !== 'image') last.content += seg.content
    else out.push({ ...seg })
  }
  return out
}
...
export function splitMessageSegments(text: string, sfx?: SfxConfig): MessageSegment[] {
  ...
  for (const match of normalized.matchAll(SEGMENT_RE)) {
    ...
    if (matched.startsWith('![')) {
      const imgMatch = matched.match(/^!\[(.*?)\]\((https?:\/\/[^\s\)]+)\)$/)
      base.push({ type: 'image', content: matched, alt: imgMatch?.[1] || 'Scene Art', url: imgMatch?.[2] || '' })
    } else {
      base.push(
        matched.startsWith('*')
