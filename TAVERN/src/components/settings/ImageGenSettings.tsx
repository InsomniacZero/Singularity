...
import { IMAGE_BACKEND_LABELS, SINGULARITY_IMAGE_MODELS, type ImageBackendId } from '@/lib/api/imageBackend'
...
const LOCAL_BACKEND_DEFAULTS: Record<'singularity' | 'custom' | 'a1111' | 'comfyui' | 'swarmui' | 'universal-gift', string> = {
  singularity: 'http://localhost:9000/v1',
  custom: 'http://localhost:9000/v1',
  'universal-gift': 'http://localhost:9000/v1',
...
export function ImageGenSettings() {
  ...
  const { models, loading, reload } = useOpenMayhemModels('IMAGES', (imageBackend as string) === 'openmayhem')
  const [preview, setPreview] = useState('')

  const autoSceneIllustration = useSettingsStore((s) => s.autoSceneIllustration)
  const setAutoSceneIllustration = useSettingsStore((s) => s.setAutoSceneIllustration)

  const isLocal = imageBackend === 'singularity' || imageBackend === 'custom' || imageBackend === 'a1111' || imageBackend === 'comfyui' || imageBackend === 'swarmui' || imageBackend === 'universal-gift'

  return (
    <SettingsPage>
      <Section
        title="Dynamic RP Scene Illustration"
        description="Generates custom scenery backdrops for each scene as the RP progresses via Singularity Gateway, updating both the chat and Visual Novel stage."
      >
        <label className="flex items-center justify-between cursor-pointer py-1">
          <div>
            <div className="text-sm font-medium text-text">Auto-Illustrate Every Scene</div>
            <div className="text-xs text-text-muted">Generates cinematic visual novel artwork in the background per text/scene turn</div>
          </div>
          <input
            type="checkbox"
            checked={autoSceneIllustration}
            onChange={(e) => setAutoSceneIllustration(e.target.checked)}
      ...
      >
        <SelectField
          ...
          onChange={(e) => {
            ...
            if (backend in LOCAL_BACKEND_DEFAULTS) {
              patch.imageBackendBaseUrl = LOCAL_BACKEND_DEFAULTS[backend as keyof typeof LOCAL_BACKEND_DEFAULTS]
              if (backend === 'singularity' || backend === 'universal-gift') {
                patch.imageBackendModel = 'gemini-3.8-flash'
              }
            }
        ...
        {(imageBackend === 'singularity' || imageBackend === 'universal-gift') && (
          <p className="mb-2 text-xs text-text-muted">
            Connected to <strong>Singularity Gateway</strong> (:9000). Generates portraits, transparent character sprites, and scenery backdrops via your local Singularity AI fleet (Gemini, ChatGPT, Grok, GLM).
          </p>
        )}

        {imageBackend === 'custom' && (
          <p className="mb-2 text-xs text-text-muted">
            Connects to any custom OpenAI-compatible <code className="font-mono">/v1/images/generations</code> endpoint.
          </p>
        )}
        ...
        {(isLocal || imageBackend === 'novelai-image') && (
          <TextField
            ...
            placeholder={
              imageBackend === 'singularity' || imageBackend === 'universal-gift'
                ? SINGULARITY_IMAGE_MODELS[0]
                : 'Leave blank to use whatever is already loaded'
            }
            list={
              imageBackend === 'singularity' || imageBackend === 'universal-gift'
                ? 'singularity-image-models'
                : undefined
            }
          />
        )}
        {(imageBackend === 'singularity' || imageBackend === 'universal-gift') && (
          <datalist id="singularity-image-models">
            {SINGULARITY_IMAGE_MODELS.map((m) => (
