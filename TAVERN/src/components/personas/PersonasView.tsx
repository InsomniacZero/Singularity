      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-5 lg:grid-cols-4">
        {personas.map((p) => (
          <button
            ...
            className="group rounded-xl border border-border/80 bg-bg-elevated p-4 text-left transition-all duration-200 hover:-translate-y-0.5 hover:border-accent/50 hover:shadow-xl active:scale-[0.99]"
          >
            {p.avatarDataUrl ? (
              <img src={p.avatarDataUrl} alt="" className="mx-auto mb-2 h-20 w-20 rounded-full object-cover transition-transform duration-300 group-hover:scale-105" />
            ) : (
              <div className="mx-auto mb-2 flex h-20 w-20 items-center justify-center rounded-full bg-bg-sunken text-xl font-bold text-text-muted">
                {p.name.slice(0, 1).toUpperCase()}
              </div>
            )}
            <div className="truncate text-center text-sm font-semibold text-text group-hover:text-accent transition-colors">{p.name}</div>
          </button>
        ))}
        {personas.length === 0 && (
          <EmptyState
