...
import { Download, Flame, ImagePlus, Plus, Sparkles, X } from 'lucide-react'
...
import { assignExpressionWarmth } from '@/lib/characters/assignWarmth'
...
export function CharacterEditor({
  ...
  const [assigningWarmth, setAssigningWarmth] = useState(false)
  const chatBackendClient = useChatBackendClient()
  ...
  const handleExportSpritesZip = () => {
    const entries: { name: string; dataUrl: string }[] = []
    for (const exp of allExpressions) {
      const key = keyFor(exp.id)
      const dataUrl = sprites[key]
      if (dataUrl) {
        const safeName = exp.label.toLowerCase().replace(/[^a-z0-9_-]/g, '_')
        entries.push({ name: `${safeName}.png`, dataUrl })
      }
      const variants = spriteVariants[key] ?? []
      variants.forEach((vUrl, idx) => {
        if (vUrl) {
          const safeName = exp.label.toLowerCase().replace(/[^a-z0-9_-]/g, '_')
          entries.push({ name: `${safeName}_var${idx + 1}.png`, dataUrl: vUrl })
        }
      })
    }

    if (entries.length === 0) {
      toastError('No sprites available to export yet. Generate or upload some first!')
      return
    }

    const charName = (form.name || 'character').trim().replace(/[^a-zA-Z0-9_-]/g, '_')
    const filename = `${charName}_sprites.zip`
    downloadZip(filename, entries)
    toastSuccess(`Exported ${entries.length} sprite(s) to ${filename}`)
  }

  const handleAutoAssignWarmth = async () => {
    if (assigningWarmth) return
    const hasPersonality = !!(form.personality?.trim() || form.description?.trim())
    if (!hasPersonality) {
      toastInfo('Please add a personality or description for this character first so warmth can be tailored to them.')
      return
    }

    setAssigningWarmth(true)
    try {
      const result = await assignExpressionWarmth(
        form,
        allExpressions.map((e) => ({ id: e.id, label: e.label })),
        chatBackendClient,
      )

      const updates: Record<string, number> = {}
      for (const [expId, warmth] of Object.entries(result.unlocks)) {
        updates[keyFor(expId)] = warmth
      }

      setSpriteUnlocks((prev) => ({ ...prev, ...updates }))
      toastSuccess(
        `Assigned warmth to ${Object.keys(result.unlocks).length} expressions (${result.archetypeSummary})`,
      )
    } catch (e) {
      toastError(errorMessage(e))
    } finally {
      setAssigningWarmth(false)
    }
  }
    ...
    >
      ...
      {tab === 'vn' && (
        ...
        >
          ...
          <div className="mb-4 rounded-xl bg-bg-sunken/60 p-3">
            <div className="mb-2 flex flex-wrap items-center gap-1.5">
              ...
              {[{ id: BASE_OUTFIT_ID, label: 'Base' }, ...outfits].map((o) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-lg px-2.5 py-1 text-[11px] leading-none transition-colors ${
                      active ? 'bg-accent text-accent-text font-medium' : 'text-text-muted hover:bg-bg-elevated hover:text-text'
                    }`}
                  >
                    {o.label}
              ...
              <input
                ...
                className="ml-1 w-44 rounded-lg bg-bg px-2.5 py-1 text-[11px] text-text outline-none ring-1 ring-transparent transition-shadow focus:ring-accent/40"
              />
                        ...
                        {knownFlags.map((f) => {
                          ...
                          return (
                            <button
                              ...
                              className={`rounded-md px-2 py-0.5 text-[10px] leading-none transition-colors ${
                                on ? 'bg-accent/20 text-accent ring-1 ring-accent/40 font-medium' : 'text-text-muted hover:bg-bg-elevated'
                              }`}
                            >
                              {f.label}
                            </button>
          ...
          <div className="mb-4 flex flex-wrap items-center gap-2">
            ...
            <Button
              variant="secondary"
              onClick={handleAutoAssignWarmth}
              disabled={assigningWarmth}
              className="flex items-center gap-1.5"
              title="Automatically calculate and assign unlock warmth (0-100) for each expression based on this character's personality and defense mechanisms"
            >
              <Flame size={14} strokeWidth={2} className={`text-accent ${assigningWarmth ? 'animate-spin' : ''}`} />
              {assigningWarmth ? 'Assigning warmth...' : 'Assign warmth by personality'}
            </Button>
            <Button
              variant="secondary"
              onClick={handleExportSpritesZip}
              className="flex items-center gap-1.5"
              title="Download all character expression sprites as a ZIP archive"
            >
              <Download size={14} strokeWidth={2} />
              Export all sprites (.zip)
            </Button>
          </div>
              ...
              <div key={exp.id} className="group relative flex flex-col items-center gap-1">
                ...
                {(exp.custom || sprites[keyFor(exp.id)]) && (
                  <button
                    ...
                    className="absolute -right-1 -top-1 hidden h-5 w-5 items-center justify-center rounded-md bg-bg-elevated text-text-muted hover:text-danger group-hover:flex"
                  >
                    <X size={11} strokeWidth={2.5} />
                  </button>
                )}
      ...
      {showExpressionSetDialog && (
        <GenerateExpressionSetDialog
          expressions={allExpressions.map((exp) => ({ id: exp.id, label: exp.label, hasSprite: !!sprites[keyFor(exp.id)] }))}
          initialPrompt={form.description ? `portrait of ${form.name || 'a character'}, ${form.description}`.slice(0, 300) : ''}
          characterPortrait={avatarDataUrl}
          characterName={form.name}
          onGenerated={(expressionId, dataUrl) => setSprites((s) => ({ ...s, [keyFor(expressionId)]: dataUrl }))}
