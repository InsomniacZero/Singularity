...
export function Chip({
  ...
  return (
    <button
      ...
      disabled={disabled}
      aria-pressed={on}
      className={`rounded-lg px-2.5 py-1 text-xs transition-all active:scale-[0.97] disabled:opacity-40 ${
        on ? TONE_ON[tone] : 'border border-border/80 text-text-muted hover:border-border hover:bg-bg-sunken hover:text-text'
      }`}
    >
      {children}
    </button>
