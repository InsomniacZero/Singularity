...
const TONE_CLASSES: Record<NonNullable<IconButtonProps['tone']>, { base: string; active: string }> = {
  chrome: { base: 'text-text-muted hover:bg-bg-sunken hover:text-text border border-transparent', active: 'bg-accent/15 text-accent border border-accent/30' },
  glass: { base: 'text-white/75 hover:bg-white/15 hover:text-white border border-transparent', active: 'bg-white/20 text-white border border-white/20' },
}
...
export function IconButton({
  ...
  return (
    <button
      ...
      className={`flex shrink-0 items-center justify-center rounded-lg transition-all duration-150 active:scale-[0.96] select-none disabled:cursor-not-allowed disabled:opacity-40 ${
