...
function RailCount({ children }: { children: React.ReactNode }) {
  return (
    <span className="rounded-md bg-current/15 px-1.5 py-0.5 text-[10px] font-medium tabular-nums opacity-80">{children}</span>
  )
}
            ...
            <div className="mb-3 flex flex-wrap gap-2">
              {activeFacts.map((f) => (
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-lg border border-border/60 bg-bg-elevated px-2.5 py-1 text-xs text-text transition-colors hover:border-danger/40 hover:text-danger"
                >
                  {f.text}
