...
const INPUT_CLASS =
  'flex-1 rounded-lg border border-border/80 bg-bg-sunken px-3 py-2 text-sm text-text outline-none transition-all placeholder:text-text-muted/60 focus:border-accent/60 focus:ring-1 focus:ring-accent/50'
...
/** Every cloud/custom option the welcome screen's compact picker offers */
...
const DEFAULT_HOSTED_PROVIDER = KNOWN_CHAT_PROVIDERS.find((p) => p.id === 'singularity') || KNOWN_CHAT_PROVIDERS[0]
...
const COMMON_LOCAL_URLS = [
  'http://localhost:9000/v1', // Singularity Gateway (Unified Local AI Fleet)
  ...
  return (
    <div className="flex flex-1 items-center justify-center overflow-y-auto p-8">
      <div className="w-full max-w-lg py-10">
        ...
        {/* 1. Connection */}
        <div className="mt-8 rounded-xl border border-border/80 bg-bg-elevated p-5 shadow-sm">
        ...
        {/* 2. First chat */}
        <div className="mt-4 rounded-xl border border-border/80 bg-bg-elevated p-5 shadow-sm">
          <div className="mb-3 text-sm font-semibold text-text">Start your first chat</div>
          ...
          {charactersLoading ? (
            ...
            <div className="h-14 animate-pulse rounded-lg bg-bg-sunken" />
          ) : featured ? (
            <>
              <div className="flex items-center gap-3">
                {featured.avatarDataUrl ? (
                  <img src={featured.avatarDataUrl} alt="" className="h-14 w-14 shrink-0 rounded-lg object-cover" />
                ) : (
                  <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-lg bg-bg-sunken text-lg font-bold text-text-muted">
                    {featured.card.name.slice(0, 1).toUpperCase()}
                  </div>
                )}
                <div className="min-w-0">
                  <div className="text-sm font-semibold text-text">{featured.card.name}</div>
