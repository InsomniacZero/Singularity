...
import { ChevronDown, ChevronsRight, FileText, Image as ImageIcon, Loader2, Paperclip, RefreshCw, Send, Sparkles, Square, Undo2, Wand2, X } from 'lucide-react'
...
interface ComposerProps {
  ...
  onIllustrate?: () => Promise<void> | void
      ...
      <div
        className={
          vn
            ...
            : 'mx-auto max-w-chat rounded-xl border border-border/80 bg-bg-sunken p-3 transition-all focus-within:border-accent/50 focus-within:bg-bg-elevated focus-within:shadow-sm'
        }
      >
        ...
        <div className={`flex items-center justify-between px-0.5 pt-0.5 ${fillHeight ? 'mt-auto shrink-0' : ''}`}>
          <div className="flex items-center gap-1">
            {turnPolicyHint ? (
              <span
                ...
                className={`mr-1 rounded-lg px-2.5 py-1.5 text-xs ${vn ? 'bg-white/10 text-white/70' : 'bg-bg-elevated text-text-muted'}`}
              >
                {turnPolicyHint}
              </span>
            ) : (
              replyAsOptions.length > 1 && (
                <CustomSelect
                  ...
                  className="mr-1 min-w-[7rem] max-w-[10rem]"
                >
            ...
            <button
              ...
              className={`flex h-8 w-8 items-center justify-center rounded-lg transition-colors disabled:opacity-40 ${iconBtnClass}`}
            >
            ...
            {onIllustrate && (
              <button
                ...
                disabled={disabled || isGenerating || isIllustrating}
              ...
              >
                {isIllustrating ? (
                  <Loader2 size={16} strokeWidth={1.75} className="animate-spin text-accent" />
                ) : (
          ...
          {isGenerating ? (
            <Button variant="danger" onClick={onAbort} className="flex items-center gap-1.5">
