...
import { currentExpressionFrom } from '@/lib/vn/stickyScene'
...
export function ChatWindow({
  ...
  const [isIllustratingScene, setIsIllustratingScene] = useState(false)
  const [dismissedChoicesId, setDismissedChoicesId] = useState<string | null>(null)

  const handleIllustrateScene = async () => {
    if (isIllustratingScene) return
    setIsIllustratingScene(true)
    try {
      const lastChar = [...messages].reverse().find((m) => m.role === 'char')
      const targetText = draft.trim() || lastChar?.text || 'Two characters talking intimately'
      const charName = character?.card?.name || 'Aria'
      const locationName = chat?.scene?.location || 'scene'
      ...
      if (res.ok) {
        ...
        if (data?.url) {
          if (lastChar) {
            await messagesApi.update(lastChar.id, { images: [data.url] })
          }
          if (chat?.id) {
      ...
      console.warn('Illustrate error:', e)
      toastError(errorMessage(e))
    } finally {
      setIsIllustratingScene(false)
    }
  }
  ...
  useEffect(() => {
    if (lastCharScene?.mood || lastCharScene?.background) {
  ...
  const reactivePortraitExpression = currentExpressionFrom(messages, character?.id)
  ...
  const activeChoices = (() => {
    ...
    if (dismissedChoicesId === last.id) return null
    return last
  })()
  ...
  const choiceListNode = (variant: 'default' | 'vn') =>
    activeChoices && (
      <ChoiceList
        ...
        onDismiss={() => setDismissedChoicesId(activeChoices.id)}
      />
    )
  ...
  const composerNode = (variant: 'default' | 'vn') => (
    <Composer
      ...
      onIllustrate={handleIllustrateScene}
      isIllustrating={isIllustratingScene}
  ...
  return (
    ...
    <div className="relative flex flex-1 flex-col min-w-0 overflow-hidden">
      {showFirstReplyTip && (
        ...
        <div className="fixed inset-x-4 bottom-6 z-40 mx-auto max-w-sm rounded-xl border border-border/80 bg-bg-elevated/95 p-4 shadow-xl backdrop-blur-md sm:inset-x-auto sm:right-6">
          <button
            onClick={dismissFirstReplyTip}
            aria-label="Dismiss tip"
            className="absolute right-2 top-2 flex h-6 w-6 items-center justify-center rounded-lg text-text-muted transition-colors hover:bg-bg-sunken hover:text-text"
          >
      ...
      {showScene && (
        <ScenePanel
          scene={chat.scene}
          locationArt={chat.locationArt}
          onDeleteLocationArt={async (locKey) => {
            const fresh = await chatsApi.get(chat.id)
            const updated = { ...(fresh?.locationArt || {}) }
            delete updated[locKey]
            await chatsApi.update(chat.id, { locationArt: updated })
          }}
