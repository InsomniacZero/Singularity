    ...
    return (
      ...
      <div className="hidden w-14 shrink-0 flex-col items-center gap-1 border-r border-border bg-bg-elevated py-4 md:flex">
        <button
          ...
          className="mb-2 flex h-9 w-9 items-center justify-center rounded-lg text-text-muted transition-colors hover:bg-bg-sunken hover:text-text active:scale-[0.96]"
        >
        ...
        <button
          ...
          title="New chat"
          aria-label="New chat"
          className="mb-1 flex h-9 w-9 items-center justify-center rounded-lg text-accent transition-colors hover:bg-accent/15 active:scale-[0.96]"
        >
          ...
          {chats.map((chat) => {
            ...
            return (
              <button
                ...
                className={`flex h-9 w-9 items-center justify-center rounded-lg transition-all active:scale-[0.96] ${
                  isActive ? 'border border-accent/40 bg-accent/20 ring-1 ring-accent/30' : 'border border-transparent hover:bg-bg-sunken'
                }`}
              >
                {character?.avatarDataUrl ? (
                  <img src={character.avatarDataUrl} className="h-9 w-9 rounded-lg object-cover" />
                ) : (
                  <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-bg-sunken text-[10px] text-text-muted">
          ...
          return (
            <div key={chat.id} className="relative mb-1">
              <button
                ...
                disabled={isBusy}
                className={`group flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left transition-all duration-150 disabled:opacity-50 ${
                  activeChatId === chat.id
                    ? 'border border-accent/40 bg-accent/15 text-text shadow-sm'
                    : 'border border-transparent hover:border-border/60 hover:bg-bg-sunken/80 text-text/90'
                }`}
              >
                <div className="relative shrink-0">
                  {character?.avatarDataUrl ? (
                    ...
                    <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-bg-sunken text-xs font-medium text-text-muted">
                      {(character?.card.name ?? '?').slice(0, 2).toUpperCase()}
                    </div>
                  )}
                  {presence && (
                ...
                <div className="min-w-0 flex-1">
                  {isRenaming ? (
                    <input
                      ...
                      className="w-full rounded-md bg-bg-elevated px-1.5 py-0.5 text-sm text-text outline-none ring-1 ring-accent/40"
                    />
                  ) : (
              ...
              {!isRenaming && (
                <button
                  ...
                  className={`absolute right-1.5 top-1/2 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-md text-text-muted transition-colors hover:bg-bg-elevated hover:text-text ${
              ...
              {isMenuOpen && (
                <>
                  ...
                  <div className="absolute right-1.5 top-10 z-50 w-52 overflow-hidden rounded-xl border border-border/80 bg-bg-elevated/95 py-1.5 shadow-2xl backdrop-blur-md">
                    <button
