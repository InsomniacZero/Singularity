...
export function ChoiceList({ choices, onPick, onRefresh, refreshing, onDismiss, variant = 'default' }: ChoiceListProps) {
  return (
    <div
    ...
    >
      {choices.map((choice, i) => {
        ...
        return (
          <button
            ...
            className={`flex shrink-0 items-center gap-2 rounded-lg py-1.5 pl-3 pr-3.5 text-left text-sm transition-colors ${CHIP_CLASSES[variant]}`}
          >
      ...
      <button
        ...
        className={`flex shrink-0 items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs transition-colors disabled:opacity-40 ${REFRESH_CLASSES[variant]}`}
      >
      ...
      {onDismiss && (
        <button
          ...
          className={`flex shrink-0 items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs transition-colors ${REFRESH_CLASSES[variant]}`}
        >
