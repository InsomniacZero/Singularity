...
import { DEFAULT_EXPRESSION_IDS, resolveExpressionSprite } from '@/lib/vn/expressions'
import { getUnlockedExpressionIds } from '@/lib/vn/unlocks'
import { useExpressionProgression } from '@/lib/hooks/useExpressionProgression'
import { currentOutfitFrom } from '@/lib/vn/outfits'
import { currentExpressionFrom, currentTaggedBackgroundFrom, currentSceneImageFrom } from '@/lib/vn/stickyScene'
...
export function VNStage({
  ...
  const activeSpeakerMember = cast.find((m) => m.id === activeSpeakerId)
  ...
  const baseExpression = scene?.expression || currentExpressionFrom(messages, activeSpeakerId)

  // Dynamic emotional progression across long or multi-beat replies: transitions from opening
  // smirk to subsequent pensive/questioning/vulnerable sprites as the reader progresses.
  const activeSpeakerAffection = activeSpeakerId ? Math.max(0, Math.min(100, getRelationshipTrack(chat, activeSpeakerId).affection ?? 0)) : affection
  const unlockedExpressions = activeSpeakerMember
    ? getUnlockedExpressionIds(activeSpeakerMember, activeSpeakerAffection)
    : DEFAULT_EXPRESSION_IDS

  const { currentExpression: progressedExpression } = useExpressionProgression({
    messageId: lastCharMsg?.id,
    text: displayText,
    initialExpression: baseExpression,
    allowedExpressionIds: unlockedExpressions,
    revealedTextLength: revealedDialogueText.length,
    typewriterActive,
  ...
  const castMembers = cast.map((member) => {
    ...
    const memberExpression = isActive ? progressedExpression : currentExpressionFrom(messages, member.id)
    ...
    const spriteUrl = isActive
      ...
      : resolveExpressionSprite(member.sprites, member.spriteUnlocks, member.avatarDataUrl, memberExpression, memberAffection, undefined, variantOptions)
    return {
      id: member.id,
  ...
  const effectiveTaggedBg = scene?.background || currentTaggedBackgroundFrom(messages)
  const resolvedBackground = resolveSceneBackground({
    taggedBackground: effectiveTaggedBg,
    chat,
    world,
    affection,
    narration,
  ...
  const currentLocKey = (chat.scene?.location || effectiveTaggedBg || sceneBackground || '').trim().toLowerCase()
  const retainedMessageImage = currentSceneImageFrom(messages, effectiveTaggedBg || chat.scene?.location || undefined)
  const backgroundUrl =
    lastCharMsg?.images?.[0] ||
    retainedMessageImage ||
    resolvedBackground.url ||
    (chat?.locationArt && currentLocKey ? chat.locationArt[currentLocKey] : undefined)
  ...
  const handleIllustrateLine = async () => {
    if (!lastCharMsg || illustrating) return
    setIllustrating(true)
    try {
      const locName = chat.scene?.location || scene?.background || sceneBackground || 'scene'
      const normLoc = locName.trim().toLowerCase()
      const res = await fetch('/api/scene/illustrate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          text: lastCharMsg.text,
          location: locName,
          chatId: chat.id,
          timePhase: chat.scene?.timePhase,
          force: true, // User manual click triggers fresh re-roll!
        }),
      })
      if (res.ok) {
        const data = (await res.json()) as { url?: string }
        if (data?.url) {
          await messagesApi.update(lastCharMsg.id, { images: [data.url] })
          const fresh = await chatsApi.get(chat.id)
          const updated = { ...(fresh?.locationArt || {}), [normLoc]: data.url }
          await chatsApi.update(chat.id, { locationArt: updated })
          toastSuccess('✨ Scene illustrated with Nano Banana!', { chime: true })
        } else {
          toastError('No image returned from Nano Banana')
        }
      } else {
        toastError(`Illustration failed: HTTP ${res.status}`)
      }
    } catch (err) {
      console.warn('VN illustrate error:', err)
        ...
        {lastCharMsg && !isStreamingThis && !showUserAsCurrent && (
          <>
            ...
            <button
              ...
              className={`flex h-7 w-7 items-center justify-center rounded-lg transition-colors hover:bg-white/10 ${speakState !== 'idle' ? 'text-accent' : 'text-white/70'}`}
            >
              {speakState === 'loading' ? (
            ...
            <button
              ...
              className={`relative flex h-7 w-7 items-center justify-center rounded-lg text-[11px] font-medium transition-colors hover:bg-white/10 ${
            ...
            <button
              ...
              className={`flex h-7 w-7 items-center justify-center rounded-lg text-xs transition-colors hover:bg-white/10 ${lastCharMsg.pinned ? 'text-accent' : 'text-white/70'}`}
            >
              <Star size={14} strokeWidth={2} fill={lastCharMsg.pinned ? 'currentColor' : 'none'} />
            </button>
            <button
              onClick={handleIllustrateLine}
              disabled={illustrating}
              title="Illustrate scene"
              aria-label="Illustrate scene"
              className={`flex h-7 w-7 items-center justify-center rounded-lg transition-colors hover:bg-white/10 ${
                illustrating ? 'text-accent animate-spin' : 'text-white/70 hover:text-accent'
              }`}
            >
              {illustrating ? <Loader2 size={14} strokeWidth={2} /> : <Sparkles size={14} strokeWidth={2} />}
            </button>
          </>
        )}
      </>
  ...
  const dialogueCaption = showUserAsCurrent ? (
    <>
      {lastUserMsg!.intimacyAction && (
        <span
          className="mt-2.5 flex w-fit items-center gap-1 rounded-md bg-white/10 px-2 py-0.5 text-[10px] font-medium text-white/70"
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-3 top-3 z-20 flex flex-col-reverse items-end gap-2 sm:inset-x-4 sm:top-4 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
        <div className="vn-glass min-w-0 self-start overflow-hidden rounded-xl text-white sm:max-w-[58%] sm:self-auto">
        ...
        <div className="vn-glass flex h-9 shrink-0 items-center gap-1 rounded-xl px-1.5">
          {onBack && (
            <>
              <button
                onClick={onBack}
                ...
                className="flex h-7 w-7 items-center justify-center rounded-lg text-white/85 transition-colors hover:bg-white/15 hover:text-white md:hidden"
              >
          ...
          {onToggleAutoAdvance && (
            <button
              ...
              className={`relative flex h-7 w-7 items-center justify-center rounded-lg transition-colors hover:bg-white/15 ${
          ...
          <button
            ...
            className="flex h-7 w-7 items-center justify-center rounded-lg text-white/85 transition-colors hover:bg-white/15 hover:text-white disabled:opacity-30"
          >
          ...
          <button
            ...
            aria-label="Hide UI"
            className="flex h-7 w-7 items-center justify-center rounded-lg text-white/85 transition-colors hover:bg-white/15 hover:text-white"
          >
          ...
          <button
            ...
            className="flex h-7 items-center gap-1.5 rounded-lg px-2 text-xs text-white/85 transition-colors hover:bg-white/15 hover:text-white sm:pr-3"
          >
        ...
        <>
          ...
          >
            {!triggeredCgEntry && artHint && character && (
              <div className="absolute inset-x-0 top-[26%] z-20 flex justify-center px-6">
                <div className="relative max-w-sm rounded-xl border border-dashed border-white/25 bg-black/45 px-5 py-4 text-center text-[12px] leading-relaxed text-white/80 backdrop-blur-sm">
                  <button
                    type="button"
          ...
          <div className="vn-stack relative z-10 mt-auto pb-2">
            <div className="flex flex-col gap-2">
              {locationCaption && (
                <span className="px-1 text-xs font-medium text-white/50">{locationCaption}</span>
              )}
              {assistSlot}
          ...
          {!inlineInput && (
            <div className="vn-stack relative z-10 pb-3 sm:pb-5">
              <div className="vn-glass rounded-xl px-3 py-1.5">{composerSlot}</div>
            </div>
          )}
          </>
          )}
          ...
          {activeChoiceData && vnChoiceStyle === 'centered' && !hideUI && (
            <VNCenteredChoices
              ...
              onRefresh={activeChoiceData.onRefresh}
              refreshing={activeChoiceData.refreshing}
              onDismiss={activeChoiceData.onDismiss}
            />
          )}
        </>
      )}
