        ...
        <div className="space-y-1">
          {trashed.map((chat) => {
            ...
            return (
              <div
                key={chat.id}
                className="flex items-center gap-2.5 rounded-lg px-2.5 py-2 transition-colors hover:bg-bg-sunken border border-transparent hover:border-border/60"
              >
                <div className="shrink-0">
                  {character?.avatarDataUrl ? (
                    <img src={character.avatarDataUrl} className="h-9 w-9 rounded-lg object-cover" />
                  ) : (
                    <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-bg-sunken text-xs font-semibold text-text-muted">
