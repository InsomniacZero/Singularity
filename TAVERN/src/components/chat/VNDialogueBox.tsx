    ...
    >
      ...
      {utilities && !writing && (
        <div className="pointer-events-none absolute -top-1 right-4 z-20 flex justify-end opacity-0 transition-opacity duration-200 focus-within:opacity-100 group-hover/vnbox:opacity-100 sm:right-6">
          <div className="vn-glass pointer-events-auto flex items-center gap-0.5 rounded-xl px-2 py-1">{utilities}</div>
        </div>
      )}

      {/* Whose box it is reads off the nameplate */}
      <div
        className={`vn-box vn-glass-solid relative overflow-hidden rounded-xl transition-shadow duration-200 ${
