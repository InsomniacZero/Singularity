...
15. **High-Definition WebGL Fluid Simulation & Autonomous Flow**:
    ...
    - *Lesson*: For image generation loading, render with GPU-accelerated WebGL at native Retina display resolution (`canvas.width = clientWidth * dpr`) to ensure silky-smooth, razor-sharp fluid curves. Keep fluid movement strictly **autonomous** (no pointer/touch controls). In Light Mode, keep the background pure white (`#ffffff`) with `#d97757` terracotta and white fluid folding and merging into each other. In Dark Mode, use dark obsidian (`#0d0c0b`) with luminous terracotta streams. Pair with "Creating image" header, aspect-ratio viewport, and frosted progress pill. On completion, `finish()` cleanly deallocates WebGL shaders and the final image pop-fades in.
