...
15. **High-Definition WebGL Fluid Simulation & TV Grain Prevention**:
    ...
    - *Lesson*: For image generation loading, render with GPU-accelerated WebGL using **analytical harmonic vortex potential flow** ($\sin, \cos, \text{atan}, \text{length}$). These are $C^\infty$ mathematically continuous, guaranteed 100% artifact-free, and impossible to produce noise or TV grain. Render at native Retina resolution (`canvas.width = clientWidth * dpr`). Keep motion strictly autonomous. In Light Mode, keep the background pure white (`#ffffff`) with `#d97757` terracotta and white fluid folding and merging into each other. In Dark Mode, use dark obsidian (`#0d0c0b`) with luminous terracotta streams. Pair with "Creating image" header, aspect-ratio viewport, and frosted progress pill. On completion, `finish()` cleanly deallocates WebGL shaders and the final image pop-fades in.
